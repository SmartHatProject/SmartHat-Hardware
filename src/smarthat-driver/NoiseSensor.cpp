...
float NoiseSensor::readSensor() {
    ...
    // Debug: Print raw sensor value and voltage
...
void NoiseSensor::setNumSamples(int samples) {
    if (samples > 0) {
        _numSamples = samples;
        delete[] _samples;  // Free the old array
        _samples = new float[_numSamples];  // Allocate new array
        _sampleIndex = 0;  // Reset the sample index
    }
}